...
Window::Window(WindowOpenGlConfig config) {
  ...
  windowHandle = glfwCreateWindow(static_cast<int>(config.width), static_cast<int>(config.height), config.title.c_str(), monitor, nullptr);
...
void Window::setCursorPosition(Position<double> cursorPosition) { glfwSetCursorPos(windowHandle, cursorPosition.x, cursorPosition.y); }
...
void Window::setClipboardContents(const std::string &contents) { glfwSetClipboardString(windowHandle, contents.c_str()); }
...
void Window::setDecorated(bool decorated) { glfwSetWindowAttrib(windowHandle, GLFW_DECORATED, decorated ? GLFW_TRUE : GLFW_FALSE); }

void Window::setResizable(bool resizable) { glfwSetWindowAttrib(windowHandle, GLFW_RESIZABLE, resizable ? GLFW_TRUE : GLFW_FALSE); }

void Window::setFloating(bool floating) { glfwSetWindowAttrib(windowHandle, GLFW_FLOATING, floating ? GLFW_TRUE : GLFW_FALSE); }
...
CursorBehavior Window::getCursorBehavior() const { return static_cast<CursorBehavior>(glfwGetInputMode(windowHandle, GLFW_CURSOR)); }

bool Window::isStickyKeys() const { return glfwGetInputMode(windowHandle, GLFW_STICKY_KEYS) == GLFW_TRUE; }

bool Window::isStickyMouseButtons() const { return glfwGetInputMode(windowHandle, GLFW_STICKY_MOUSE_BUTTONS) == GLFW_TRUE; }
...
bool Window::isRawMouseMotionEnabled() const { return glfwGetInputMode(windowHandle, GLFW_RAW_MOUSE_MOTION) == GLFW_TRUE; }

void Window::setCursorBehavior(CursorBehavior behavior) { glfwSetInputMode(windowHandle, GLFW_CURSOR, static_cast<int>(behavior)); }

void Window::setStickyKeys(bool sticky) { glfwSetInputMode(windowHandle, GLFW_STICKY_KEYS, sticky ? GLFW_TRUE : GLFW_FALSE); }
...
void Window::setLockModifierKeys(bool lock) { glfwSetInputMode(windowHandle, GLFW_LOCK_KEY_MODS, lock ? GLFW_TRUE : GLFW_FALSE); }
...
void Window::framebufferSizeGLFWCallback(GLFWwindow *window, int width, int height) {
  auto self = reinterpret_cast<Window *>(glfwGetWindowUserPointer(window));
  if (self->previousCallbacks.glfWframebuffersizefun != nullptr) { self->previousCallbacks.glfWframebuffersizefun(window, width, height); }
